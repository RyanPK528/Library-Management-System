...
/**
 * Implements the "Return Book" operation, allowing users to return a borrowed book
 */
public class ReturnBook implements IOOperation {

	@Override
	/**
	 * Performs the "Return Book" operation, creating a GUI for the user to input book name
	 */
						...
						if (b.getBook().getName().matches(name.getText().toString()) &&
							...
							if (b.getDaysLeft()>0) {
								JOptionPane.showMessageDialog(new JFrame(), "You are late!\n"
										+ "You have to pay "+Math.abs(b.getDaysLeft()*10)+"k as fine");
							}
							book.setBrwcopies(book.getBrwcopies()+1);
							database.returnBook(b, book, i);
							JOptionPane.showMessageDialog(new JFrame(), "Book has been returned\nThank you!");
							frame.dispose();
							break;
						} else {
